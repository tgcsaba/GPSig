...
def signature_kern_first_order(M, num_levels, difference = True, reverse = False):
    """
    ...
    for i in range(2, num_levels+1):
        R = M * tf.cumsum(tf.cumsum(R, exclusive=True, reverse=reverse,  axis=1), exclusive=True, reverse=reverse, axis=-1)
        K.append(tf.reduce_sum(R, axis=(1, -1)))

    return tf.stack(K, axis=0)

def signature_kern_higher_order(M, num_levels, order=2, difference = True, reverse = False):
    """
    ...
    for i in range(2, num_levels+1):
        ...
        R_next[0, 0] = M * tf.cumsum(tf.cumsum(tf.add_n(R.flatten().tolist()), exclusive=True, reverse=reverse, axis=1), exclusive=True, reverse=reverse, axis=-1)
        for j in range(2, d+1):
            R_next[0, j-1] = 1 / tf.cast(j, settings.float_type) * M * tf.cumsum(tf.add_n(R[:, j-2].tolist()), exclusive=True, reverse=reverse, axis=1)
            R_next[j-1, 0] = 1 / tf.cast(j, settings.float_type) * M * tf.cumsum(tf.add_n(R[j-2, :].tolist()), exclusive=True, reverse=reverse, axis=-1)
...
def signature_kern_tens_vs_seq_first_order(M, num_levels, difference = True, reverse = False):
    """
    ...
    for i in range(1, num_levels+1):
        ...
        k += 1
        for j in range(1, i):
            R = M[k] * tf.cumsum(R, exclusive=True, reverse=reverse, axis=2)
            k += 1
...
def signature_kern_first_order_lr_feature(U, num_levels, rank_bound, sparsity = 'sqrt', seeds = None, difference = True, reverse = False):
    """
    ...
    P = U
    for i in range(2, num_levels+1): 
        P = tf.cumsum(P, axis=1, exclusive=True, reverse=reverse)
        if seeds is None:
