...
from gpflow.inducing_variables.inducing_variables import InducingPointsBase, Kuu, Kuf
...
class SignatureInducing(InducingPointsBase):
    ...
    def __init__(self, Z, num_levels, learn_weights=False):
        ...
        if learn_weights:
            self.W = Parameter(np.tile(np.eye(self.__len__())[None, ...], [num_levels, 1, 1]))
...
def Kuu_Kuf_Kff(feat, kern, X_new, *, jitter=0.0, full_f_cov=False):
    with params_as_tensors_for(feat):
        ...
        Kzz += jitter * tf.eye(len(feat))
        if full_f_cov:
            Kxx += jitter * tf.eye(tf.shape(X)[0])
        else:
            Kxx += jitter
    return Kzz, Kzx, Kxx